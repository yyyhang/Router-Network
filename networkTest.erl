...
-export([probeNetwork/2, runTest/0, verifyNetwork/2]).
...
verifyNetwork(RootPid, Graph) ->
    ...
    io:format("*** Verifying network containing nodes: "
	      "~w~n",
	      [NodeNames]),
    NodeMap = ets:new(undef, [private]),
    NodeAssocs = [{Name,
		   element(1, probeNetwork(RootPid, Name))}
		  || Name <- NodeNames],
    Undefined = [Name
		 || {Name, Pid} <- NodeAssocs, Pid == undef],
    if length(Undefined) /= 0 ->
	   io:format("*** Network contains undefined node(s): "
		     "~w~n",
		     [Undefined]);
       true ->
...
checkTrace(Dest, [Node, Next | Nodes], Graph) ->
    ...
    case [EdgeDest
	  || {EdgeDest, Label} <- Edges,
	     lists:member(Dest, Label)]
	of
      [EdgeDest]
	  when EdgeDest == Next ->         % next node ok in graph
